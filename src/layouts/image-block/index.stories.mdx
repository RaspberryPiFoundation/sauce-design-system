...
Avoid images going "full-bleed" on the `x` axis. Full bleed on one of the edges of the `y` axis is fine, but never both. Vertical image aligment is accomplished with the `.sl-block--image-bottom` and `.sl-block--image-top` modifier classes.
...
In the following examples you can see the quiet area on the images illustrated with a transparent red box. This is enabled with the `.sl-block--debug` modifier. **Do not use this classname in production**.
