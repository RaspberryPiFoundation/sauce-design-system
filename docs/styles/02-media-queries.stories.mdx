...
What's more, the web is much more fluid that "viewport width". We can—and *should*—employ a greater range of media queries to ensure that our experiences offer the best experience we can to any user, on any device.

Using a greater mix of all the [media queries available to us in modern web browsers](https://developer.mozilla.org/en-US/docs/Web/CSS/Media_Queries/Using_media_queries), we can better cater for a user's accessibility needs, device preferences, network conditions, ambient lighting, input method, and more. We can even detect if JavaScript is available, **in CSS**.
...
FUN FACT: One day we may be able to detect a user's connection speed with another media query, and use a combination to determine if a user has both the screen pixel density *and* the connection bandwidth to handle downloading a higher-resolution image.
